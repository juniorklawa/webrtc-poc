...
export default function JoinScreen({ setScreen, screens, roomId }) {
  ...
  useEffect(() => {
    ...
    socket.on("newUser", (data) => {
      console.log("currentOffer", data);
      setCurrentOffer(data);
    });

    socket.on("callerCandidates", (data) => {
      console.log(data)
      setCurrentCallerCandidates(data);
    });
  }, []);
  ...
  const joinCall = async (id) => {
    ...
    // pega a oferta criada no firebase e coloca como remoteDescription
    // const offer = roomSnapshot.data().offer;

    await localPC.setRemoteDescription(new RTCSessionDescription(currentOffer));
    ...
    currentCallerCandidates.forEach(async (data) => {
